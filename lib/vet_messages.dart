...
// ─────────────────────────────────────────────
//  VetMessagesScreen  –  AgriSynth Mesajlarım
// ─────────────────────────────────────────────
class VetMessagesScreen extends StatefulWidget {
  final Map<String, dynamic> userData;

  const VetMessagesScreen({super.key, required this.userData});

  @override
  State<VetMessagesScreen> createState() => _VetMessagesScreenState();
}

class _VetMessagesScreenState extends State<VetMessagesScreen>
    with TickerProviderStateMixin {
  // ── Renkler (vet_panel.dart mavi palet) ──────────────────
  ...
  Future<void> _loadChats() async {
    ...
    if (currentUserId.isEmpty) {
      debugPrint('Vet chat load error: Kullanıcı id bulunamadı.');
      if (mounted) {
        setState(() {
          _chats = [];
          _loadingChats = false;
    ...
    try {
      // Senin chats tablon participant_ids kullanmıyor.
      // Gerçek şema: user1_id / user2_id.
      ...
      if (mounted) setState(() => _chats = enriched);
    } catch (e) {
      debugPrint('Vet chat load error: $e');
      if (mounted) {
  ...
  Future<Map<String, dynamic>> _fetchUserById(String userId) async {
    ...
    final selectAttempts = <String>[
      // Senin mevcut users şeman
      'id, username, email, ad, soyad, telefon, sehir, rol, created_at, avatar_url',
      // Eski/alternatif şemalarla da uyumlu kalsın
      ...
      'id, first_name, last_name, phone_number, email, avatar_url, city, role',
    ...
    for (final columns in selectAttempts) {
      try {
        ...
        // Seçilen kolon setinde olmayan alan varsa sonraki denemeye geç.
      }
    }

    debugPrint('Vet user fetch error: users tablosunda uygun kolon seti bulunamadı.');
  ...
  Future<List<Map<String, dynamic>>> _fetchMessagesByChatId(
    ...
    try {
      ...
      // is_edited kolonu yoksa eski messages şemasını da destekle.
      if (limit == null) {
        final data = await SupabaseService()
            .client
            .from('messages')
  ...
  Future<void> _openChat(Map<String, dynamic> chat) async {
    ...
    try {
      ...
      debugPrint('Vet message load error: $e');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  Future<void> _editMessage(Map<String, dynamic> msg) async {
          ...
          content: TextField(
            ...
            decoration: InputDecoration(
              ...
              border: OutlineInputBorder(
                ...
                borderSide: BorderSide(color: softBlue.withOpacity(0.4)),
              ),
    ...
    try {
      ...
      debugPrint('Vet edit message error: $e');
  ...
  Future<void> _deleteMessage(Map<String, dynamic> msg) async {
    ...
    try {
      ...
      debugPrint('Vet delete message error: $e');
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                if (!isDeleted)
                  ListTile(
                    leading: const Icon(Icons.copy_rounded, color: primaryBlue),
                ...
                if (isMe && !isDeleted) ...[
                  ListTile(
                    leading: const Icon(Icons.edit_rounded, color: primaryBlue),
  ...
  void _showSnack(String message, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? Colors.red.shade500 : primaryBlue,
        behavior: SnackBarBehavior.floating,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
  ...
  String _otherRole(Map<String, dynamic> chat) {
    final u = Map<String, dynamic>.from(chat['other_user'] ?? {});
    return _formatRoleLabel(_userField(u, ['rol', 'role', 'user_role']));
  }
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: softBlue.withOpacity(0.45)),
                boxShadow: [
                  BoxShadow(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Icon(Icons.verified_user_rounded, size: 13, color: primaryBlue),
  ...
  Widget _profileDetailRow({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border(
          bottom: isLast ? BorderSide.none : BorderSide(color: softBlue.withOpacity(0.24)),
        ),
      ),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: VetAppDrawer(
        ...
        currentPage: VetDrawerPage.messages,
      ),
      body: SafeArea(
        child: Column(
          children: [
            ...
            children: [
              Builder(
                builder: (drawerContext) => InkWell(
                  ...
                  child: Container(
                    ...
                    child: Icon(
                      _selectedChat != null ? Icons.arrow_back_ios_new_rounded : Icons.menu_rounded,
                      color: darkText,
                      size: 18,
                    ),
                  ),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: primaryBlue.withOpacity(0.18), width: 2),
                ),
                child: ClipRRect(
          ...
          _selectedChat == null
              ? const IgnorePointer(
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(Icons.eco_rounded, color: primaryBlue, size: 23),
                      SizedBox(width: 8),
                      Text(
                        'AgriSynth',
                        style: TextStyle(
              ...
              : GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        _miniAvatar(_otherAvatar(_selectedChat!), _otherName(_selectedChat!), radius: 18),
                        ...
                        Icon(Icons.keyboard_arrow_down_rounded, size: 18, color: mutedText),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _buildChatTile(Map<String, dynamic> chat) {
    ...
    final lastTs = (chat['last_message'] as Map<String, dynamic>?)?['created_at'] ?? chat['created_at'];
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? primaryBlue.withOpacity(0.06) : cardWhite,
  ...
  Widget _buildDateDivider(dynamic ts) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Expanded(child: Divider(color: softBlue.withOpacity(0.4))),
          Padding(
                  ...
                  children: [
                    ...
                    if (isUpdating) ...[
                      ...
                      SizedBox(
                        ...
                        child: CircularProgressIndicator(
                          strokeWidth: 1.5,
                          color: isDeleted || !isMe ? primaryBlue : Colors.white,
                        ),
                      ),
                    ] else if (isMe) ...[
  ...
  Widget _buildInputBar() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          top: BorderSide(color: softBlue.withOpacity(0.3), width: 1),
        ),
      ),
      child: Row(
        children: [
  ...
  Widget _miniAvatar(String url, String name, {double radius = 22}) {
    return Container(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        border: Border.all(color: softBlue.withOpacity(0.5), width: 1.5),
      ),
      child: ClipOval(
