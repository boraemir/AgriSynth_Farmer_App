...
// ─────────────────────────────────────────────
//  UserMessagesScreen  –  AgriSynth Mesajlarım
// ─────────────────────────────────────────────
...
class _UserMessagesScreenState extends State<UserMessagesScreen>
  ...
  Future<void> _loadChats() async {
    ...
    try {
      ...
      await _openInitialChatIfNeeded(enriched);
    } catch (e) {
  ...
  Future<void> _openInitialChatIfNeeded(List<Map<String, dynamic>> chats) async {
    final targetUserId = (widget.initialChatUserId ?? '').trim();
    final currentUserId = _userId;

    if (_initialChatHandled || targetUserId.isEmpty || currentUserId.isEmpty) return;
    ...
    for (final chat in chats) {
      if ((chat['other_user_id'] ?? '').toString() == targetUserId) {
        targetChat = chat;
        break;
      }
    }
    ...
    if (targetChat == null) {
      try {
        final first = currentUserId.compareTo(targetUserId) <= 0 ? currentUserId : targetUserId;
        final second = currentUserId.compareTo(targetUserId) <= 0 ? targetUserId : currentUserId;
        ...
        Map<String, dynamic> chatRaw;
        if (existing == null) {
          final created = await SupabaseService()
              .client
              .from('chats')
              .insert({
                'user1_id': first,
              ...
              .select('id, created_at, user1_id, user2_id')
              .single();
          chatRaw = Map<String, dynamic>.from(created);
        } else {
          chatRaw = Map<String, dynamic>.from(existing);
        }

        final otherUser = await _fetchUserById(targetUserId);
        final lastMessages = await _fetchMessagesByChatId(
          (chatRaw['id'] ?? '').toString(),
          ascending: false,
          limit: 1,
        ...
        if (mounted) {
          setState(() {
            _chats = [
              targetChat!,
              ..._chats.where((item) => item['id'] != targetChat!['id']),
            ];
          });
        }
      } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: UserAppDrawer(
        ...
        currentPage: UserDrawerPage.messages,
      ),
      body: SafeArea(
        child: Column(
          children: [
