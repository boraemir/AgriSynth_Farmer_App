...
class UserJobsScreen extends StatefulWidget {
  ...
  const UserJobsScreen({super.key, required this.userData});
...
class _UserJobsScreenState extends State<UserJobsScreen> {
  ...
  Future<void> _startChatWithPoster(Map<String, dynamic> job) async {
    ...
    final otherUserId = (job['user_id'] ?? '').toString().trim();

    if (currentUserId.isEmpty) {
      _showSnackBar('Kullanıcı oturumu bulunamadı. Lütfen tekrar giriş yapın.', isError: true);
      return;
    }

    if (otherUserId.isEmpty) {
      _showSnackBar('İlan sahibinin kullanıcı bilgisi bulunamadı.', isError: true);
      return;
    }

    if (otherUserId == currentUserId) {
      _showSnackBar('Kendi ilanınız için mesaj başlatamazsınız.', isError: true);
      return;
    }

    try {
      final first = currentUserId.compareTo(otherUserId) <= 0 ? currentUserId : otherUserId;
      final second = currentUserId.compareTo(otherUserId) <= 0 ? otherUserId : currentUserId;
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    _avatarOrInitial(user['avatar_url'], fullName, size: 58),
                    const SizedBox(width: 14),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Text(
                            'Başvuru: ${_formatDate(app['created_at'])}',
                            style: const TextStyle(
                              color: textMuted,
                              fontSize: 12,
                              fontWeight: FontWeight.w600,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: UserAppDrawer(
        userData: widget.userData,
        currentPage: UserDrawerPage.jobs,
      ),
      body: SafeArea(
          ...
          children: [
            Row(
              children: [
                ...
                const Spacer(),
                _avatarOrInitial(_avatarUrl, _fullName, size: 42),
              ],
            ),
            const IgnorePointer(
              child: Row(
            ...
            Container(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: isOwn ? null : () => _applyToJob(job),
                      icon: Icon(isOwn ? Icons.check_circle_outline : Icons.send_rounded),
                      label: Text(isOwn ? 'İlanım' : 'Başvur'),
                      style: ElevatedButton.styleFrom(
                        elevation: 0,
                        minimumSize: const Size.fromHeight(46),
                        backgroundColor: isOwn ? const Color(0xFFDDE3DF) : primaryGreen,
                        foregroundColor: isOwn ? textMuted : Colors.white,
  ...
  Widget _buildApplicantCard(Map<String, dynamic> app) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          _avatarOrInitial(user['avatar_url'], fullName, size: 48),
          ...
          if (pending) ...[
            _roundIconButton(
              icon: Icons.search_rounded,
              color: info,
              onTap: () => _showApplicantDetail(app),
            ),
            const SizedBox(width: 7),
            _roundIconButton(
              icon: Icons.check_rounded,
              color: primaryGreen,
              onTap: () => _updateApplicationStatus((app['id'] ?? '').toString(), 'accepted'),
            ),
            const SizedBox(width: 7),
            _roundIconButton(
              icon: Icons.close_rounded,
              color: danger,
              onTap: () => _updateApplicationStatus((app['id'] ?? '').toString(), 'rejected'),
            ),
          ] else
            _roundIconButton(
              icon: Icons.search_rounded,
...
class _JobFormScreenState extends State<_JobFormScreen> {
  static const Color primaryGreen = _UserJobsScreenState.primaryGreen;
  static const Color pageBg = _UserJobsScreenState.pageBg;
  static const Color cardBg = _UserJobsScreenState.cardBg;
  static const Color textDark = _UserJobsScreenState.textDark;
  static const Color textMuted = _UserJobsScreenState.textMuted;
  static const Color borderColor = _UserJobsScreenState.borderColor;
  static const Color danger = _UserJobsScreenState.danger;
